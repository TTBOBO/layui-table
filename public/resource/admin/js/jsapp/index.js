(function ($) {
    $.learuntab = {
        ...
        activeTab: function () {
            ...
            var currentId = index != 0 ? $(this).data('id') : $(this).data('id');      //选项卡url
            var menuName = $.trim($(this).text());   //选项卡名字
            var dataId = $(this).attr('data-num');   //选项卡id
            ykp.setSessionStorage('navBar',JSON.stringify({title:menuName,url:currentId,id:dataId}));
            ykp.setSessionStorage('iframeId',"iframe"+dataId);
            var index = $(this).index();
...
//							console.log($(this).attr('id'));
        ...
        addTab: function () {
            ...
            var flag = true;
          	ykp.setSessionStorage('navBar',JSON.stringify({title:menuName,url:dataUrl,id:dataId}));
          	ykp.setSessionStorage('iframeId',"iframe"+dataId);
            if (dataUrl == undefined || $.trim(dataUrl).length == 0 || dataUrl == "#" || dataUrl == "javascript:void(0)") {
                return false;
            }
            ...
            //当没有创建iframe时  创建
            if (flag) {
                var str = '<a href="javascript:;" class="active menuTab" data-num="'+ dataId +'" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-remove"></i></a>';
