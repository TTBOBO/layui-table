 ...
 * 把二维多级分类数组转换树状结构
 */
function getChildTree($cateIds, $parentId = 0, $primaryKey = 'id', $parentField = 'parent_id')
{
    if (!is_array($cateIds) || empty($parentField)) return $cateIds;
    $returnData = [];
    foreach ($cateIds as $ck => $cv) {
        if ($cv[$parentField] == $parentId) {
            unset($cateIds[$ck]);
            $newParentId = $cv[$primaryKey];
            $childs = getChildTree($cateIds, $newParentId, $primaryKey, $parentField);
            if (!empty($childs)) $cv['childs'] = $childs;
            $returnData[$cv[$primaryKey]] = $cv;
        }
    }
 ...
 * @return type
 */
function getTreeData($list, $pid = 0)
{
    $data = [];
    if (!empty($list)) {
        $tmp = [];
        ...
        foreach ($tmp as $v) {
            if ($pid == $v['parent_id']) {
                $data[] = &$tmp[$v['id']];
            } elseif (isset($tmp[$v['parent_id']])) {
                $tmp[$v['parent_id']]['children'][] = &$tmp[$v['id']];
            }
        }
    }
    return $data;
