...
class Cz_user extends Modelbase {
    ...
    {
        parent::__construct();
        $this->_table = 'om_cz_order';
    }

    public function setQuery($it, $select = "*", $filter = NULL)
    {
        if ($select == '*') {
            $select = array_merge($this->getCols('om_cz_order'), $this->getCols('um_user'), $this->getCols('um_company'));
            //$select[] = "table.name as 'table.name'";
        }
        $it->db->select($select);
        $it->db->distinct(TRUE);
        $it->db->from($this->_table);
        $it->db->join("um_user", "um_user.uid = om_cz_order.uid", "left");
