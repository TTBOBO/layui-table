...
class Om_refund extends Modelbase {
    ...
    {
        if ($select == '*') {
            $select = array_merge($this->getCols($this->_table), $this->getCols('um_user'), $this->getCols('am_admin'));
        ...
        $it->db->join("am_admin", "am_admin.aid = {$this->_table}.aid", "left");
        if (!empty($filter)) {
            $it->db->where($filter);
        }
        return $it;
